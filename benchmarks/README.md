...
Ground truth lives in `expected-findings/v0.1-insecure.json`. The test compares total candidates, per-rule counts, and verifier verdict counts. The current fixture expects 33 candidates: 26 confirmed and 7 `needs_validation`. The latter are intentional: five absence-based backup observations and two public hosts without a complete firewall-collector proof must not be silently confirmed.
