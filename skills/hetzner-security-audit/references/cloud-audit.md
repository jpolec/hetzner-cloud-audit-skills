...
If a token is missing, direct the operator to the target project in Hetzner Console: **Security → API tokens → Generate API token → Read**. Do not request the secret in chat or a command argument. The full value is shown only once and is bound to that project. Never test permission level with a write request. Use the project's [illustrated token guide](https://github.com/jpolec/hetzner-cloud-audit-skills/blob/main/docs/token-setup.md) for exact setup, CI, troubleshooting, and revocation steps.
