...
| `reachable_after_pivot` | Every layer admits the flow, but only after another host is compromised, which the evidence does not prove |
...
|---|---|---|
| **Install** | `npx skills add https://github.com/jpolec/hetzner-cloud-audit-skills/tree/v0.8.2 --skill hetzner-security-audit` | `uvx hetzner-audit …`, or `git clone` and `uv run` |
...
To pin a release, use `uvx --from 'git+https://github.com/jpolec/hetzner-cloud-audit-skills@v0.8.2' hetzner-audit --help`. For CI, see the [GitHub Action](#9-track-changes-in-ci).
