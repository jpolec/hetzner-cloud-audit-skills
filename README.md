...
![Network map of a real 18-server Hetzner project: trust sources on the left, servers grouped by role, public edges labelled with ports](docs/assets/network-map.png)
...
![Audit of a real Hetzner project: no database open to the Internet, one indirect pivot path, three confirmed findings](docs/assets/real-audit.png)

The [synthetic benchmark report](examples/reports/demo.md) shows what the tool finds on an intentionally insecure setup.
...
It shows firewall intent, not host or application controls. It never prints public IP addresses, but it still reveals your topology, so treat it as private. See the [example map](examples/reports/network-map.md).
