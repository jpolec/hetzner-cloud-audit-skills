...
![Network map of a real 18-server Hetzner project: trust sources on the left, servers grouped by role, public edges labelled with ports](https://raw.githubusercontent.com/jpolec/hetzner-cloud-audit-skills/main/docs/assets/network-map.png)

<sub>Real Hetzner project, generated by `hetzner-audit map` from a read-only token. Server names were replaced; public IP addresses are never rendered.</sub>
...
### What you get on a real project

![Audit of a real Hetzner project: no database open to the Internet, one indirect pivot path, three confirmed findings](https://raw.githubusercontent.com/jpolec/hetzner-cloud-audit-skills/main/docs/assets/real-audit.png)
...
hetzner-audit map --format svg --output network-map.svg       # standalone image
```

The map groups servers by role and private network and colors each server by exposure:
