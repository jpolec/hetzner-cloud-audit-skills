...
> Independent open-source project. Not affiliated with or endorsed by Hetzner, Cloudflare, or any other provider it recognizes. v0.7 is alpha: review every high-impact result before acting on it.
...
To pin a release, use `uvx --from 'git+https://github.com/jpolec/hetzner-cloud-audit-skills@v0.7.0' hetzner-audit --help`. For CI, see the [GitHub Action](#9-track-changes-in-ci).
...
| **Object Storage** | `--object-storage` (S3 GET, SigV4) or a saved file | Public ACLs, wildcard bucket policies (read or write), versioning off; public access is confirmed with one anonymous listing |
...
- **Three savings numbers, never mixed:**
  ...
  - **verified**: measured after the change with `hetzner-audit diff before.json after.json`. Collection gaps block the claim.
...
- **No SSH, no probing.** The tool never connects to your servers and never port-scans. Host evidence comes from a script you review and run yourself. The only unauthenticated request is one anonymous bucket listing, made to confirm that a bucket already looks public.
