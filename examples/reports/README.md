...
`network-map.md` is different: it is an example project derived from a real deployment, with names, sizes, locations, addressing, and ports changed. `demo.sh` cannot regenerate it, so the file is refreshed by hand.
